    ...
    >
        ...
        <Stat 
          ...
          icon={
            <svg className="h-6 w-6 text-amber-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 5v2m0 4v22m0-2h2m-2 0H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
            </svg>
          }
      ...
      <div className="mt-2 grid grid-cols-1 gap-8 lg:grid-cols-3">
        ...
        <div className="rounded-3xl bg-white p-8 shadow-xl shadow-slate-200/50 border border-[var(--border)] lg:col-span-2 relative overflow-hidden group">
          <div className="flex items-center justify-between mb-6 relative z-10">
            <div>
              <h3 className="font-bold text-2xl text-[var(--foreground)]">Live Overview</h3>
              <p className="text-[var(--muted-foreground)]">Real-time parking occupancy</p>
            </div>
          ...
          <div className="relative h-80 w-full overflow-hidden rounded-2xl shadow-inner border border-[var(--border)]">
             ...
             <div className="absolute top-1/4 left-1/4 p-2 bg-white rounded-lg shadow-lg animate-bounce duration-[3000ms]">
                <div className="text-xs font-bold text-[var(--foreground)]">Zone A</div>
                <div className="text-[10px] text-green-600 font-bold">85% Free</div>
             </div>
             <div className="absolute bottom-1/3 right-1/3 p-2 bg-white rounded-lg shadow-lg animate-bounce duration-[4000ms]">
                <div className="text-xs font-bold text-[var(--foreground)]">Zone B</div>
                <div className="text-[10px] text-amber-600 font-bold">40% Free</div>
             </div>
          </div>
        </div>
        ...
        <div className="rounded-3xl bg-white p-8 shadow-xl shadow-slate-200/50 border border-[var(--border)] flex flex-col">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-bold text-xl text-[var(--foreground)]">Zones</h3>
            <Link href="/admin/config" className="text-sm font-medium text-[var(--primary)] hover:underline">View All</Link>
          </div>
            ...
            {store.zones.map((z) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-bold text-[var(--foreground)]">{z.name}</div>
                    <div className="text-xs text-[var(--muted-foreground)]">{z.maxHours}h Limit</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="font-bold text-[var(--foreground)]">${z.ratePerHour}</div>
                  <div className="text-xs text-[var(--muted-foreground)]">/hr</div>
                </div>
              </div>
            ))}
      ...
      <div className="mt-8 rounded-3xl bg-white shadow-xl shadow-slate-200/50 border border-[var(--border)] overflow-hidden">
        <div className="p-8 border-b border-[var(--border)] flex items-center justify-between">
          <h3 className="font-bold text-xl text-[var(--foreground)]">Recent Tickets</h3>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[var(--border)]">
              {store.tickets.slice(0, 5).map((t) => (
                <tr key={t.id} className="group hover:bg-[var(--muted)]/30 transition-colors">
                  <td className="py-4 px-6 font-mono font-medium text-[var(--primary)]">#{t.id.slice(-6)}</td>
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-3">
                      <div className="h-8 w-8 rounded-full bg-gradient-to-br from-[var(--primary)] to-violet-500 flex items-center justify-center text-white text-xs font-bold">
                        {store.users.find((u) => u.id === t.driverId)?.name.charAt(0) || 'U'}
                      </div>
                      <span className="font-medium text-[var(--foreground)]">
                        {store.users.find((u) => u.id === t.driverId)?.name || 'Unknown Driver'}
                      </span>
                    </div>
                  </td>
                  <td className="py-4 px-6 font-bold text-[var(--foreground)]">{money(t.amount)}</td>
                  <td className="py-4 px-6">
                ...
                <tr>
                  <td colSpan={5} className="py-12 text-center text-[var(--muted-foreground)]">
                    <div className="flex flex-col items-center gap-2">
                      <svg className="h-10 w-10 text-[var(--border)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>
...
function Stat({ title, value, trend, trendDown, icon, color }: { title: string; value: string; trend?: string; trendDown?: boolean; icon?: React.ReactNode; color?: string }) {
  return (
    <div className={`rounded-3xl p-6 shadow-lg shadow-slate-200/50 border transition-all hover:-translate-y-1 ${color || 'bg-white border-[var(--border)]'}`}>
      <div className="flex items-start justify-between mb-4">
        <div className="p-3 rounded-2xl bg-white shadow-sm border border-[var(--border)]/50">
          {icon}
        </div>
        {trend && (
          <div className={`flex items-center gap-1 text-sm font-bold ${trendDown ? 'text-red-500' : 'text-emerald-600'}`}>
            {trendDown ? (
      ...
      <div className="text-sm font-medium text-[var(--muted-foreground)] mb-1">{title}</div>
      <div className="text-4xl font-bold tracking-tight text-[var(--foreground)]">{value}</div>
    </div>
  )
}
